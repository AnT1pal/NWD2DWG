...
| **Модель в папке без права записи** | Navisworks такую модель не открывает и рвёт связь. `.nwd` и `.nwc` программа сама берёт во временную копию; составной `.nwf` придётся перенести вручную вместе со связанными файлами |
...
### Несколько версий Navisworks

Плагин привязывается к версии API на этапе компиляции: собранный под 2026 в
2020-м не загрузится — перенаправление версий у Navisworks не выходит за
пределы своего поколения. Поэтому сборка находит **все** установленные
Navisworks и собирает плагин под каждый, а `NWD2DWG.exe` при запуске выбирает
подходящий по версии установленного Navisworks:

```
Собран плагин под Navisworks Manage 2020 (API 17): 268 КБ
Собран плагин под Navisworks Manage 2021 (API 18): 268 КБ
Собран плагин под Navisworks Manage 2026 (API 23): 268 КБ
Вложено плагинов: 3 (API 17, 18, 23)
```

Активация Navisworks для сборки не нужна — компилятору требуются только файлы
сборок. Если нужной версии на машине сборки нет, положите пять её библиотек в
`refs\<любое имя>\` — они будут подхвачены наравне с установленными:
...
> [!IMPORTANT]
> Собранный exe несёт плагины только тех версий, что были доступны при сборке.
> На чужой версии программа скажет об этом прямо и перечислит вложенные —
> вместо невнятной ошибки загрузки сборки.
